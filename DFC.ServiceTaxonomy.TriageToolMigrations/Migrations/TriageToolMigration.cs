...
namespace DFC.ServiceTaxonomy.TriageToolMigrations.Migrations
{
    public class TriageToolMigration : DataMigration
    {
        ...
        public async Task<int> UpdateFrom6Async()
        {
            try
            {
                _logger.LogInformation($"Starting step 7 of DFC.ServiceTaxonomy.TriageToolMigrations");
                await _recipeMigrator.ExecuteAsync("MigrationRecipes/placements.recipe.json", this);
                _logger.LogInformation($"Completed step 7 of DFC.ServiceTaxonomy.TriageToolMigrations");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error with step 7 of DFC.ServiceTaxonomy.TriageToolMigrations");
            }
