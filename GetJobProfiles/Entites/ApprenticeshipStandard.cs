...
namespace GetJobProfiles.Entities
{
    public class ApprenticeshipStandard
    {
        ...
        public ApprenticeshipStandard( string[] apprenticeshipStandards )
        {
        ...
        public ApprenticeshipStandard Build(
            SettingsModel jobProfileSettingsDataModel,
            ReferenceData jobProfileReferenceDataModel)
        {
            BuildReplacementsDictionary();
            var apprenticeshipStandards = ReadStandardsFromFile(jobProfileReferenceDataModel.JobProfileExcelWorkbook);
            ...
            ApprenticeshipStandardRouteContentItems = apprenticeshipStandards.Where(x => x.Route != null).SelectMany(x => x.Route).Distinct().Select(route => new ApprenticeshipStandardRouteContentItem(route, jobProfileSettingsDataModel.Timestamp, routeDictionary[route]));
            ApprenticeshipStandardContentItems = apprenticeshipStandards.Select(standard => new ApprenticeshipStandardContentItem(standard.Name, jobProfileSettingsDataModel.Timestamp, _standardsDictionary[standard.Name])
            {
                EponymousPart = new ApprenticeshipStandardPart
                {
                    ...
                    {
                        ContentItemIds = standard.Level.HasValue ? new List<string> { jobProfileReferenceDataModel.QcfLevels.QcfLevelDictionary[standard.Level.ToString()] } : new List<string>()
                    },
                    Type = new TextField(standard.Type)
                }
            });

            AssignStandardsToJobProfiles(jobProfileReferenceDataModel.JobProfileExcelWorkbook, jobProfileReferenceDataModel.JobProfiles.jobProfileContentItems);
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<Models.API.ApprenticeshipStandardApiModel> ReadStandardsFromFile(XSSFWorkbook jobProfileWorkbook)
        {
            var apprenticeshipStandardList = new List<Models.API.ApprenticeshipStandardApiModel>();
            ...
            using (var reader = new StreamReader(@"SeedData\ApprenticeshipStandards.xlsx"))
            {
                ...
                for (int i = 1; i <= sheet.LastRowNum; i++)
                {
                    ...
                    if (status.Equals("Approved for delivery", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        {
                            var apprenticeshipStandard = new Models.API.ApprenticeshipStandardApiModel
                            {
                                Name = name,
                                Reference = reference,
        ...
        private static void BuildFrameworks(XSSFWorkbook workbook, List<Models.API.ApprenticeshipStandardApiModel> apprenticeshipStandardList)
        {
            ...
            for (int i = 1; i <= sheet.LastRowNum; i++)
            {
                ...
                if (string.IsNullOrWhiteSpace(standards) && !string.IsNullOrWhiteSpace(frameworks))
                {
                    ...
                    {
                        var apprenticeshipStandard = new Models.API.ApprenticeshipStandardApiModel { Name = framework, Type = "Framework"};
