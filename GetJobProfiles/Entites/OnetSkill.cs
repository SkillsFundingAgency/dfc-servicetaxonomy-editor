...
namespace GetJobProfiles.Entities
{
    public class OnetSkill
    {
        ...
        private readonly Dictionary<string, List<ONetSkillRankApiModel>> _oNetOccupationToSkillRank = new Dictionary<string, List<ONetSkillRankApiModel>>();
        ...
        private readonly SettingsModel _jobProfileSettingsDataModel;
        private readonly ReferenceData _jobProfileReferenceDataModel;

        public OnetSkill(
            SettingsModel jobProfileSettingsDataModel,
        ...
        public OnetSkill Build()
        {
            LoadONetSkillRanks(_jobProfileReferenceDataModel.JobProfileExcelWorkbook);
            ImportTraits(
                _jobProfileReferenceDataModel.JobCategories.JobCategoryContentItemIdDictionary,
                _jobProfileReferenceDataModel.DysacExcelWorkbook,
                _jobProfileSettingsDataModel.Timestamp);

            ImportShortQuestions(_jobProfileReferenceDataModel.DysacExcelWorkbook, _jobProfileSettingsDataModel.Timestamp);
            ImportQuestionSet(_jobProfileSettingsDataModel.Timestamp);
            BuildONetSkills(_jobProfileReferenceDataModel, _jobProfileReferenceDataModel.DysacJobProfileMappingExcelWorkbook);

            return this;
        }

        public void BuildONetSkills(ReferenceData jobProfileReferenceDataModel,  XSSFWorkbook dysacWorkbook)
        {
            var sheet = dysacWorkbook.GetSheet("JP Link");
            if(sheet == null)
            {
                throw new ArgumentNullException("BuildONetSkills: DYSAC/Job Profile workbook is null");
            }
            ...
            var _oNetOccupationalCodeContentItems = jobProfileReferenceDataModel.ONetOccCodes.ONetOccupationalCodeContentItems;
        ...
        private void LoadONetSkillRanks(XSSFWorkbook jobProfileWorkbook)
        {
            ...
            for (int i = 1; i <= sheet.LastRowNum; i++)
            {
                ...
                var occCodeToSocCodeMap = _jobProfileReferenceDataModel.ONetOccCodes.ONetOccupationalCodeToSocCodeMappingDictionary;
                var occupationalCode = occCodeToSocCodeMap.ContainsKey(socCode) ? occCodeToSocCodeMap[socCode].ToUpperInvariant() : string.Empty;

                if (!string.IsNullOrEmpty(occupationalCode))
                {
                    if (!_oNetOccupationToSkillRank.ContainsKey(occupationalCode))
                    {
                        _oNetOccupationToSkillRank.Add(occupationalCode, new List<ONetSkillRankApiModel> { new ONetSkillRankApiModel { Name = skillName, Rank = rank } });
                    }
                    else
                    {
                        if (!_oNetOccupationToSkillRank[occupationalCode].Any(z => z.Name.ToUpperInvariant().Trim() == skillName.ToUpperInvariant()))
                        {
                            _oNetOccupationToSkillRank[occupationalCode].Add(new ONetSkillRankApiModel { Name = skillName, Rank = rank });
                        }
                    }
                }
            }
