...
using System.Linq;
using GetJobProfiles.Models.Spreadsheet.UniversityRequirement;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    public class UniversityRequirementSpreadsheetImporter
    {
        // The UniversityRequirement dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        public IReadOnlyDictionary<string, UniversityRequirementSpreadsheetRowModel> UniversityRequirementDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "UniversityRequirement";

        public IReadOnlyDictionary<string, UniversityRequirementSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, UniversityRequirementSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new UniversityRequirementSpreadsheetColumnIndexModel();

            // Initialise a spreadsheet row model so that we can use the column names to lookup the column indexes
            var spreadsheetRowModel = new UniversityRequirementSpreadsheetRowModel();
            ...
            IReadOnlyDictionary<string, UniversityRequirementSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            UniversityRequirementSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            UniversityRequirementSpreadsheetRowModel spreadsheetRowModel)
        {
        ...
        private UniversityRequirementSpreadsheetRowModel GetTheSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            UniversityRequirementSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)
        {
            UniversityRequirementSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new UniversityRequirementSpreadsheetRowModel();
