...
using System.Linq;
using GetJobProfiles.Models.Spreadsheet.DysacFilteringQuestion;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    public class DysacFilteringQuestionSpreadsheetImporter
    {
        // The DysacFilteringQuestion dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        public IReadOnlyDictionary<string, DysacFilteringQuestionSpreadsheetRowModel> DysacFilteringQuestionDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "FilteringQuestion";

        public IReadOnlyDictionary<string, DysacFilteringQuestionSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, DysacFilteringQuestionSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new DysacFilteringQuestionSpreadsheetColumnIndexModel();

            // Initialise a spreadsheet row model so that we can use the column names to lookup the column indexes
            var spreadsheetRowModel = new DysacFilteringQuestionSpreadsheetRowModel();
            ...
            IReadOnlyDictionary<string, DysacFilteringQuestionSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            DysacFilteringQuestionSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            DysacFilteringQuestionSpreadsheetRowModel spreadsheetRowModel)
        {
        ...
        private DysacFilteringQuestionSpreadsheetRowModel GetTheSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            DysacFilteringQuestionSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)
        {
            DysacFilteringQuestionSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new DysacFilteringQuestionSpreadsheetRowModel();
