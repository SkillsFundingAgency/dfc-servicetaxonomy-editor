...
using System.Linq;
using GetJobProfiles.Models.Spreadsheet.Location;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    public class LocationSpreadsheetImporter
    {
        // The Location dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        public IReadOnlyDictionary<string, LocationSpreadsheetRowModel> LocationDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "Location";

        public IReadOnlyDictionary<string, LocationSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, LocationSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new LocationSpreadsheetColumnIndexModel();

            // Initialise a spreadsheet row model so that we can use the column names to lookup the column indexes
            var spreadsheetRowModel = new LocationSpreadsheetRowModel();
            ...
            IReadOnlyDictionary<string, LocationSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            LocationSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            LocationSpreadsheetRowModel spreadsheetRowModel)
        {
        ...
        private LocationSpreadsheetRowModel GetTheSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            LocationSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)
        {
            LocationSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new LocationSpreadsheetRowModel();
                ...
                if (isNegative != null)
                {
                    spreadsheetRowModel.IsNegative = isNegative.StringCellValue;
                }
