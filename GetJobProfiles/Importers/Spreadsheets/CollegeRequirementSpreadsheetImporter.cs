...
using System.Linq;
using GetJobProfiles.Models.Spreadsheet.CollegeRequirement;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    public class CollegeRequirementSpreadsheetImporter
    {
        // The CollegeRequirement dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        // CollegeRequirement
        public IReadOnlyDictionary<string, CollegeRequirementSpreadsheetRowModel> CollegeRequirementDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "CollegeRequirement";

        public IReadOnlyDictionary<string, CollegeRequirementSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, CollegeRequirementSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new CollegeRequirementSpreadsheetColumnIndexModel();

            // Initialise a spreadsheet row model so that we can use the column names to lookup the column indexes
            var spreadsheetRowModel = new CollegeRequirementSpreadsheetRowModel();
            ...
            for (int i = 1; i <= spreadsheet.LastRowNum; ++i)
            {
                ...
                spreadsheetRowModel = GetCollegeRequirementSpreadsheetRowData(spreadsheetRow, spreadsheetColumnIndexModel);
            ...
            IReadOnlyDictionary<string, CollegeRequirementSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            CollegeRequirementSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            CollegeRequirementSpreadsheetRowModel spreadsheetRowModel)
        {
        ...
        private CollegeRequirementSpreadsheetRowModel GetCollegeRequirementSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            CollegeRequirementSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)
        {
            CollegeRequirementSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new CollegeRequirementSpreadsheetRowModel();
                ...
                // Info
                NPOI.SS.UserModel.ICell url = spreadsheetRow.GetCell(spreadsheetColumnIndexModel.InfoColumnIndex);
                if (url != null)
                {
                    spreadsheetRowModel.Info = url.StringCellValue;
                }
