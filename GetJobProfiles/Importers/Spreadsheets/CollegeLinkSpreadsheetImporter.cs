...
using System.Linq;
using GetJobProfiles.Models.Spreadsheet.CollegeLink;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    public class CollegeLinkSpreadsheetImporter
    {
        // The CollegeLink dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        public IReadOnlyDictionary<string, CollegeLinkSpreadsheetRowModel> CollegeLinkDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "CollegeLink";

        public IReadOnlyDictionary<string, CollegeLinkSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, CollegeLinkSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new CollegeLinkSpreadsheetColumnIndexModel();

            // Initialise a spreadsheet row model so that we can use the column names to lookup the column indexes
            var spreadsheetRowModel = new CollegeLinkSpreadsheetRowModel();
            ...
            IReadOnlyDictionary<string, CollegeLinkSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            CollegeLinkSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            CollegeLinkSpreadsheetRowModel spreadsheetRowModel)
        {
        ...
        private CollegeLinkSpreadsheetRowModel GetTheSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            CollegeLinkSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)
        {
            CollegeLinkSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new CollegeLinkSpreadsheetRowModel();
