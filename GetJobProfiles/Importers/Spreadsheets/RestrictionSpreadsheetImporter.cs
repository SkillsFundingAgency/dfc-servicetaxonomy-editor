...
using System.Linq;
using GetJobProfiles.Models.Spreadsheet.Restriction;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    class RestrictionSpreadsheetImporter
    {
        // The Restriction dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        public IReadOnlyDictionary<string, RestrictionSpreadsheetRowModel> RestrictionDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "Restriction";

        public IReadOnlyDictionary<string, RestrictionSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, RestrictionSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new RestrictionSpreadsheetColumnIndexModel();

            // Initialise a row model so that we can use the column names to get the column indexes
            var spreadsheetRowModel = new RestrictionSpreadsheetRowModel();
            ...
            IReadOnlyDictionary<string, RestrictionSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            RestrictionSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            RestrictionSpreadsheetRowModel spreadsheetRowModel)
        {
        ...
        private RestrictionSpreadsheetRowModel GetTheSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            RestrictionSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)
        {
            RestrictionSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new RestrictionSpreadsheetRowModel();
