...
using System.Linq;
using GetJobProfiles.Models.Spreadsheet.SocSkillsMatrix;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    public class SocSkillsMatrixSpreadsheetImporter
    {
        // The SocSkillsMatrix dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        public IReadOnlyDictionary<string, SocSkillsMatrixSpreadsheetRowModel> SocSkillsMatrixDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "SocSkillsMatrix";

        public IReadOnlyDictionary<string, SocSkillsMatrixSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, SocSkillsMatrixSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new SocSkillsMatrixSpreadsheetColumnIndexModel();

            // Initialise a spreadsheet row model so that we can use the column names to lookup the column indexes
            var spreadsheetRowModel = new SocSkillsMatrixSpreadsheetRowModel();
            ...
            IReadOnlyDictionary<string, SocSkillsMatrixSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            SocSkillsMatrixSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            SocSkillsMatrixSpreadsheetRowModel spreadsheetRowModel)
        {
            ...
            spreadsheetColumnIndexModel.ONetAttributeTypeColumnIndex = spreadsheet
                  .GetRow(0)
                  .Cells
                  .Single(x => x.StringCellValue == nameof(spreadsheetRowModel.ONetAttributeType))
                  .ColumnIndex;

            // RankColumnIndex
            spreadsheetColumnIndexModel.RankColumnIndex = spreadsheet
                  .GetRow(0)
                  .Cells
                  .Single(x => x.StringCellValue == nameof(spreadsheetRowModel.Rank))
                  .ColumnIndex;

            // ContextualisedColumnIndex
            spreadsheetColumnIndexModel.ContextualisedColumnIndex = spreadsheet
                  .GetRow(0)
                  .Cells
                  .Single(x => x.StringCellValue == nameof(spreadsheetRowModel.Contextualised))
                  .ColumnIndex;
            ...
            // ONetRankColumnIndex
            spreadsheetColumnIndexModel.ONetRankColumnIndex = spreadsheet
                  .GetRow(0)
                  .Cells
                  .Single(x => x.StringCellValue == nameof(spreadsheetRowModel.ONetRank))
                  .ColumnIndex;
        }

        private SocSkillsMatrixSpreadsheetRowModel GetTheSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            SocSkillsMatrixSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)
        {
            SocSkillsMatrixSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new SocSkillsMatrixSpreadsheetRowModel();
                ...
                // ONetAttributeType
                NPOI.SS.UserModel.ICell oNetAttributeType = spreadsheetRow.GetCell(spreadsheetColumnIndexModel.ONetAttributeTypeColumnIndex);
                if (oNetAttributeType != null)
                {
                    spreadsheetRowModel.ONetAttributeType = oNetAttributeType.StringCellValue;
                }

                // Rank
                NPOI.SS.UserModel.ICell rank = spreadsheetRow.GetCell(spreadsheetColumnIndexModel.RankColumnIndex);
                if (rank != null)
                {
                    spreadsheetRowModel.Rank = Convert.ToByte(rank.StringCellValue);
                }

                // Split the Title into two new columns containing the SOC code and the Skill so that we can search on the SOC code
                char[] delimiterChars = { '-', '_' };

                if (!string.IsNullOrEmpty(spreadsheetRowModel.Title))
                {
                    string[] words = spreadsheetRowModel.Title.Split(delimiterChars);

                    if (words != null && words.Length >= 2)
                    {
                        spreadsheetRowModel.SocCode = words[0];
                        spreadsheetRowModel.Skill = words[1];
                    }
                }
            }
