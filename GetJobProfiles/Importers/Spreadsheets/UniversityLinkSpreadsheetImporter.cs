...
using System.Linq;
using GetJobProfiles.Models.Spreadsheet.UniversityLink;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    class UniversityLinkSpreadsheetImporter
    {
        // The UniversityLink dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        public IReadOnlyDictionary<string, UniversityLinkSpreadsheetRowModel> UniversityLinkDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "UniversityLink";

        public IReadOnlyDictionary<string, UniversityLinkSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, UniversityLinkSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new UniversityLinkSpreadsheetColumnIndexModel();

            // Initialise a spreadsheet row model so that we can use the column names to lookup the column indexes
            var spreadsheetRowModel = new UniversityLinkSpreadsheetRowModel();
            ...
            IReadOnlyDictionary<string, UniversityLinkSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            UniversityLinkSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            UniversityLinkSpreadsheetRowModel spreadsheetRowModel)
        {
        ...
        private UniversityLinkSpreadsheetRowModel GetTheSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            UniversityLinkSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)
        {
            UniversityLinkSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new UniversityLinkSpreadsheetRowModel();
