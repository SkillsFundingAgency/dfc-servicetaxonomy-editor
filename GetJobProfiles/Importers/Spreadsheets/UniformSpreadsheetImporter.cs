...
using System.Linq;
using GetJobProfiles.Models.Recipe.Spreadsheet.Uniform;
using NPOI.XSSF.UserModel;

namespace GetJobProfiles.Importers.Spreadsheets
{
    public class UniformSpreadsheetImporter
    {
        // The Uniform dictionary of imported data in key/value format where the value is row model for the given spreadsheet
        public IReadOnlyDictionary<string, UniformSpreadsheetRowModel> UniformDictionary { get; private set; }

        // The name of the Spreadsheet to import
        private const string SpreadsheetName = "Uniform";

        public IReadOnlyDictionary<string, UniformSpreadsheetRowModel> Import(XSSFWorkbook spreadsheetWorkbook)
        {
            // Initialise the dictionary that will hold the Spreadsheet row/columns model
            var spreadsheetDictionary = new Dictionary<string, UniformSpreadsheetRowModel>();
            ...
            var spreadsheetColumnIndexModel = new UniformSpreadsheetColumnIndexModel();

            // Initialise a spreadsheet row model so that we can use the column names to lookup the column indexes
            var spreadsheetRowModel = new UniformSpreadsheetRowModel();
            ...
            {
                // Get the row data
                var speadsheetRow = spreadsheet.GetRow(i);
                ...
                spreadsheetRowModel = GetTheSpreadsheetRowData(speadsheetRow, spreadsheetColumnIndexModel);
            ...
            IReadOnlyDictionary<string, UniformSpreadsheetRowModel> readonlySpreadsheet = spreadsheetDictionary;
        ...
        private void InitialiseTheSpreadsheetColumnIndexes(
            ...
            UniformSpreadsheetColumnIndexModel spreadsheetColumnIndexModel,
            UniformSpreadsheetRowModel spreadsheetRowModel)
        {
        ...
        private UniformSpreadsheetRowModel GetTheSpreadsheetRowData(
            NPOI.SS.UserModel.IRow spreadsheetRow,
            UniformSpreadsheetColumnIndexModel spreadsheetColumnIndexModel)

        {
            UniformSpreadsheetRowModel spreadsheetRowModel = null;

            if (spreadsheetRow != null)
            {
                spreadsheetRowModel = new UniformSpreadsheetRowModel();
