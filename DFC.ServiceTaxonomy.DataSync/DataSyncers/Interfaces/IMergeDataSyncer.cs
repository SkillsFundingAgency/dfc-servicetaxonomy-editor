...
{
    public interface IMergeDataSyncer
    {
        IMergeNodeCommand MergeNodeCommand { get; }
        IDataMergeContext? DataSyncMergeContext { get; }

        Task<IAllowSync> SyncToDataSyncReplicaSetIfAllowed(
        ...
        /// <summary>
        /// We need to keep the published and preview data sync replica sets consistent.
        /// As they contain different data, a sync may fail in one and succeed in t'other,
        /// and when that happens it means the data syncs become inconsistent.
        ...
        /// but we have the data sync validation process to handle those cases.)
        ///
        ...
        /// Preview data sync fails (correctly) as location is used by preview page.
        ...
        Task<IMergeNodeCommand?> SyncToDataSyncReplicaSet();

        Task<IMergeDataSyncer?> SyncEmbedded(ContentItem contentItem);
    }
}
