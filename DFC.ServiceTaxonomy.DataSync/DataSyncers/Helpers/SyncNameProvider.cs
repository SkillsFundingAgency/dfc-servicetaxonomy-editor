...
#pragma warning disable S4136
    ...
    public class SyncNameProvider : ISyncNameProvider
    {
        ...
        private GraphSyncPartSettings? _dataSyncPartSettings;
        ...
        public string? ContentType
        {
            ...
            set
            {
                _contentType = value ?? throw new SetArgumentNullException($"Null is not an acceptable value for {nameof(ContentType)}.");
                _dataSyncPartSettings = GetDataSyncPartSettings(value);
            }
        }

        public GraphSyncPartSettings DataSyncPartSettings
        {
            get
            {
                CheckPreconditions();

                return _dataSyncPartSettings!;
            }
        }
        ...
        public async Task<IEnumerable<string>> NodeLabels()
        {
            ...
            return await NodeLabels(DataSyncPartSettings!, _contentType!);
        }

        public async Task<IEnumerable<string>> NodeLabels(string contentType)
        {
            var dataSyncPartSettings = GetDataSyncPartSettings(contentType);

            return await NodeLabels(dataSyncPartSettings, contentType);
        }

        private async Task<IEnumerable<string>> NodeLabels(GraphSyncPartSettings dataSyncPartSettings, string contentType)
        {
            //todo: rename NodeNameTransform to NodeLabelTransform
            string nodeLabel = dataSyncPartSettings.NodeNameTransform switch
            {
                "ContentType" => contentType,
                "$\"esco__{ContentType}\"" => $"esco__{contentType}",
                _ => await TransformOrDefault(dataSyncPartSettings.NodeNameTransform, contentType, contentType)
            };
        ...
        {
            var dataSyncPartSettings = GetDataSyncPartSettings(destinationContentType);

            return await TransformOrDefault(
                dataSyncPartSettings.CreateRelationshipType,
        ...
        public async Task<string> PropertyName(string name)
        {
            ...
            string propertyName = DataSyncPartSettings!.PropertyNameTransform switch
            {
                "Value" => name,
                _ => await TransformOrDefault(DataSyncPartSettings!.PropertyNameTransform, name, _contentType!)
            };
        ...
        public string IdPropertyName()
        {
            ...
            return IdPropertyName(DataSyncPartSettings!);
        }

        public string IdPropertyName(string contentType)
        {
            return IdPropertyName(GetDataSyncPartSettings(contentType));
        }
        ...
        private string IdPropertyName(GraphSyncPartSettings dataSyncPartSettings)
        {
            return dataSyncPartSettings.IdPropertyName ?? "userId";
        }
        ...
        public async Task<string> GenerateIdPropertyValue()
        {
            ...
            return await GenerateIdPropertyValue(_contentType!, DataSyncPartSettings!);
        }

        public async Task<string> GenerateIdPropertyValue(string contentType)
        {
            return await GenerateIdPropertyValue(contentType, GetDataSyncPartSettings(contentType));
        }

        private async Task<string> GenerateIdPropertyValue(string contentType, GraphSyncPartSettings dataSyncPartSettings)
        {
            string newGuid = Guid.NewGuid().ToString("D");

            return dataSyncPartSettings.GenerateIdPropertyValue switch
            {
                ...
                _ => await TransformOrDefault(dataSyncPartSettings.GenerateIdPropertyValue, newGuid, contentType)
            };
        }
        ...
        public object? GetAndConvertIdPropertyValue(
            JObject dataSyncContent,
            IContentItemVersion contentItemVersion,
            params IContentItemVersion[] fromContentItemVersions)
        {
            object? idValue = dataSyncContent[ContentIdPropertyName]?.ToObject<object?>();
        ...
        {
            if (!_dataSyncPartSettings!.PreexistingNode)
                return contentItemVersion.ContentApiBaseUrl;

            if (_dataSyncPartSettings.PreExistingNodeUriPrefix == null)
                throw new InvalidOperationException("Bad DataSyncPart settings: PreexistingNode is true, but PreExistingNodeUriPrefix is null.");

            return _dataSyncPartSettings.PreExistingNodeUriPrefix;
        }
        ...
        public GraphSyncPartSettings GetDataSyncPartSettings(string contentType)
        {
            ContentTypeDefinition? contentTypeDefinition =
                ContentDefinitionHelper.GetTypeDefinitionCaseInsensitive(contentType, _contentDefinitionManager, false);

            if (contentTypeDefinition == null)
                throw new DataSyncException($"Attempt to get {nameof(DataSyncPartSettings)} for {contentType}, but it doesn't have a {nameof(ContentTypeDefinition)}.");
            ...
            if (contentTypePartDefinition == null)
                throw new DataSyncException($"Attempt to get {nameof(DataSyncPartSettings)} for {contentType}, but it doesn't have a {nameof(GraphSyncPart)}.");
